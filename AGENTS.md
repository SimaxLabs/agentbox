...
- `ai_kit/cli.py` lazily imports the browser UI so the base CLI remains dependency-free. Do not introduce FastAPI or uvicorn imports into the core import path.
- `ai_kit/web.py` serves the loopback-only browser UI.
- The source launcher defaults to the adjacent repository `ai-kit.json`; installed entry points prefer `AI_KIT_CONFIG`, then a current-directory config, then `ai_kit/default_config.json`.
...
- `logo.png` is the source artwork; `ai_kit/static/logo.png` is the optimized web copy. Regenerate it when the source changes.
