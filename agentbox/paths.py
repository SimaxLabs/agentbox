    ...
    if sys.platform == "win32":  # pragma: no cover - exercised on Windows.
        ...
        if base:
            ...
            if candidate.is_absolute():
                return candidate / "AgentBox/config.json"
        return Path.home() / "AppData/Roaming/AgentBox/config.json"
    xdg_config_home = os.environ.get("XDG_CONFIG_HOME")
    if xdg_config_home:
        candidate = Path(xdg_config_home).expanduser()
        if candidate.is_absolute():
            return candidate / "agentbox/config.json"
    return Path.home() / ".config/agentbox/config.json"


def default_config_path(repository_default: Path | None = None) -> Path:
    override = os.environ.get("AGENTBOX_CONFIG")
    if override:
    ...
    working_copy = Path.cwd() / "agentbox.json"
